...
SCLPL ships a VS Code extension source tree in `editors/vscode`. It recognizes only
...
```powershell
py -3.12 -m pip install "sclpl[editor]"
cd editors/vscode
npm ci
npm run package
```
...
package` in `editors/vscode`. Python language-service tests live in `tests/language`.
