...
export class SclplClient {
  ...
  async start(): Promise<void> {
    ...
    const serverOptions: ServerOptions = async () => {
      const args = python === "py" ? ["-3", "-m", "sclpl.language.server", "--stdio"] : ["-m", "sclpl.language.server", "--stdio"];
  ...
  private async unavailable(): Promise<void> {
    ...
    const action = await vscode.window.showWarningMessage('SCLPL language server is unavailable in this Python environment. Install: pip install "sclpl[editor]"', "Retry", "Select Python");
